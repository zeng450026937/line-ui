var Line = (function (exports, Vue) {
  ...
  const on = (
    ...
    event,
    listener,
    opts = { passive: false, capture: false }
  ) => {
  ...
  const createKeyframeStylesheet = (keyframeName, keyframeRules, element) => {
    ...
    const stylesheet = (element.ownerDocument || document).createElement(
      'style'
    );
  ...
  var ActionSheet = /*#__PURE__*/ createComponent$4({
    ...
    computed: {
      ...
      cancelAction() {
        return this.normalizedActions.find(
          (action) => action.role === 'cancel'
        );
      },
    },
  ...
  var Alert = /*#__PURE__*/ createComponent$5({
    ...
    beforeMount() {
      ...
      this.$on('animation-enter', (baseEl, animate) => {
        const builder =
          mode === 'md' ? mdEnterAnimation$1 : iosEnterAnimation$1;
        animate(builder(baseEl));
      });
      this.$on('animation-leave', (baseEl, animate) => {
        const builder =
          mode === 'md' ? mdLeaveAnimation$1 : iosLeaveAnimation$1;
        animate(builder(baseEl));
      });
    },
  ...
  const getTimezoneOffset = (localDate, timeZone) => {
    ...
    const tzDateTime = new Date(
      localDate.toLocaleString('en-US', { timeZone })
    );
      ...
      FORMAT_KEYS.forEach((format) => {
        if (word === format.f) {
          if (word === FORMAT_A || word === FORMAT_a) {
            ...
            if (
              (formats.indexOf(FORMAT_h) < 0 &&
                formats.indexOf(FORMAT_hh) < 0) ||
  ...
  var datetime = /*#__PURE__*/ createComponent$w({
    ...
    methods: {
      ...
      generateColumns() {
        ...
        let template =
          this.pickerFormat || this.displayFormat || DEFAULT_FORMAT;
      ...
      validateColumns(columns) {
        ...
        if (yearCol) {
          ...
          if (
            !yearCol.options.find((col) => col.value === today.getFullYear())
          ) {
  ...
  const swipeShouldReset = (isResetDirection, isMovingFast, isOnResetZone) => {
    ...
    return (
      (!isMovingFast && isOnResetZone) || (isResetDirection && isMovingFast)
    );
  };
      ...
      onEnd(gesture) {
        ...
        let restingPoint =
          this.openAmount > 0
            ...
            : -this.optsWidthLeftSide; // Check if the drag didn't clear the buttons mid-point
  ...
  var listView = /*#__PURE__*/ createComponent$Y({
    ...
    methods: {
      ...
      async onUpdate(force = false) {
        ...
        const clientSize = this.horizontal
          ? this.clientWidth
          : this.clientHeight;
        ...
        newFrom = binarySearch(
          ...
          this.incremental
            ? Math.min(
                count - 1,
                Math.ceil(leftBoundary / this.minimumItemSize)
              )
            : lastTo
        );
  ...
  const menuOverlayAnimation = (menu) => {
    ...
    return baseAnimation(isIos).addAnimation([
      menuAnimation,
      backdropAnimation,
    ]);
  };
  ...
  const sanitizeDOMString = (untrustedString) => {
    try {
      ...
      blockedTags.forEach((blockedTag) => {
        const getElementsToRemove = documentFragment.querySelectorAll(
          blockedTag
        );
        for (
  ...
  var segment = /*#__PURE__*/ createComponent$19({
    ...
    async mounted() {
      ...
      this.inToolbarColor =
        this.$el.closest('.line-toolbar.line-color') !== null;
