...
export const isMobile = (win: Window) =>
...
export const isCordova = (win: any): boolean =>
...
export const isHybrid = (win: Window) =>
  isCordova(win) || isCapacitorNative(win);

export const isMobileWeb = (win: Window): boolean =>
  isMobile(win) && !isHybrid(win);

export const isElectron = (win: Window): boolean =>
  testUserAgent(win, /electron/i);

export const isPWA = (win: Window): boolean =>
  !!(
