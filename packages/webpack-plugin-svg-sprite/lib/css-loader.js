...
const { helpers, ReplacementGenerator: generator } = require('./utils');
...
module.exports = function (content, sourcemap, meta = {}) {
  ...
  const ast =
    meta && meta.ast && meta.ast.type === 'postcss'
      ? meta.ast.root
      : postcss.parse(content, {
          from: loader.resourcePath + loader.resourceQuery,
        });
  ...
  Promise.all(resolvePromises)
    .then((data) => {
      data.forEach(({ decl, helper }) => {
        ...
        const svgLoaderRule = matchedRules.find(
          (r) => r.value.loader === LOADER_PATH
        );
        ...
        const { spriteType, selector } = merge(
          {},
          plugin.config,
          svgLoaderOpts
        );
