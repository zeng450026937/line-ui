...
module.exports = function (content, sourcemap, meta = {}) {
  ...
  const symbolId =
    typeof config.symbolId === 'function'
  ...
  // TODO
  // Pass context & loader to runtimeGenerator
  // Parse query to gen different runtime, eg. runtime for css / js
  const runtime = new config.runtimeGenerator(symbol, config).generate();

  callback(null, runtime, sourcemap, { ...meta, symbol, config });
};
